...
class Cube3DArray : public Cube {
 private:
  void rotate_face(const int face_index) {
    ...
    for (int i = 0; i < 3; i++) cube[face_index][0][i] = temp[2 - i][0];
    for (int i = 0; i < 3; i++) cube[face_index][i][2] = temp[0][i];
    for (int i = 0; i < 3; i++) cube[face_index][2][2 - i] = temp[i][2];
    for (int i = 0; i < 3; i++) cube[face_index][2 - i][0] = temp[2][2 - i];
  };
 ...
 public:
  ...
  bool operator==(const Cube3DArray &other) const {
    for (int i = 0; i < other.total_faces; i++) {
      for (int j = 0; j < other.total_rows; j++) {
        for (int k = 0; k < other.total_cols; k++) {
  ...
  std::unique_ptr<Cube> clone() const override {
    return std::make_unique<Cube3DArray>(*this);
  }
};
...
struct hash<Cube3DArray> {
  size_t operator()(const Cube3DArray &cube) const noexcept {
    ...
    for (int i = 0; i < cube.total_faces; i++) {
      for (int j = 0; j < cube.total_rows; j++) {
        for (int k = 0; k < cube.total_cols; k++) {
